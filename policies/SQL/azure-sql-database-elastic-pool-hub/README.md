...
# Create the Policy Assignment
policyAssignment=$(az policy assignment create --name 'azure-sql-database-elastic-pool-hub-assignment' --display-name ' Assignment' --scope `echo $scope | jq '.id' -r` --policy `echo $policyDefinition | jq '.name' -r` --params "$policyParameters")
